...
def make_board():
    ...
    # These rows mate with spiq_a J2 and J3 respectively. HEADER_X puts the
    # nominal 2.54 mm header body flush with dualflash's west edge. The
    # default SIL direction preserves spiq_a's north-to-south pin numbering.
    ...
    dip.DIP8(brd.DC((DIP_X, NORTH_DIP_Y)))
    dip.DIP8(brd.DC((DIP_X, SOUTH_DIP_Y)))

    # Add component placement and routing above this point. CuFlow's Board
    # creates two inner layers by default; discard them so the fabrication
    # output contains only top and bottom copper.
    for layer in ("GL2", "GL3"):
        assert brd.layers[layer].preview().is_empty
